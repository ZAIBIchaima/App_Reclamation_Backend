...
@Service
public class ExportSourceExecutionService {
	...
	public ByteArrayInputStream pdf(List<SourceExecution> sourceExecutions) {
		...
		try {
			...
			Paragraph paragraph_1 = new Paragraph("جهة التنفيذ",titlefont);
			...
			//make column title
			Stream.of("عدد جهة التنفيذ"," جهة التنفيذ ").forEach(headerTitle -> {
			...
			for(SourceExecution inf: sourceExecutions) {
				//n° type
				...
				//nom type  
				PdfPCell nomRecCell = new PdfPCell(new Phrase(inf.getNom(),font));
