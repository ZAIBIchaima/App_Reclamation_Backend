...
@Service
public class ExportSourceService {
	...
	public  ByteArrayInputStream pdf(List<Entite> sources) {
		...
		try {
			...
			Paragraph paragraph_1 = new Paragraph("قائمة الجهات",titlefont);
			...
			//make column title
			Stream.of("اسم االجهة","العنوان ","الوظيفة","البريد الالكتروني","المصلحة").forEach(headerTitle -> {
			...
			for(Entite inf: sources) {
				//nom Employe
				PdfPCell nomCell = new PdfPCell(new Phrase(inf.getNom(),font));
